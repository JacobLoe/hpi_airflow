...
with DAG('feature_extraction', default_args=default_args,
    ...
    feature_extraction = (DockerOperator(
        ...
        command='/data/ /file_mappings.tsv '+vid +
                ' --file_extension {{ti.xcom_pull(key="extractor_file_extension")}}'
                ' --force_run {{ti.xcom_pull(key="feature_extraction_force_run")}}',
    ...
    init >> shot_detection >> image_extraction >> feature_extraction
